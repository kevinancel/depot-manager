...
function genFacture(dossier, clients) {
  ...
  if (premiereEntree && derniereSortie) {
    ...
    if (stock.montant > 0) {
      lignes.push({ desc: "Stockage " + stock.joursFactures + "j facturés / " + stock.jours + "j total (3j offerts) — " + totalPal + " pal. × " + stock.taux + "€", qty: stock.joursFactures, pu: totalPal * (stock.taux || 0), total: stock.montant });
      total += stock.montant;
    }
  }
  ...
  if (dossier.depotage > 0) { const mt = dossier.depotage * TARIFS.depotage_vrac; lignes.push({ desc: "Dépotage vrac unité (" + dossier.depotage + " colis)", qty: dossier.depotage, pu: TARIFS.depotage_vrac, total: mt }); total += mt; }
...
function PageNouveauDossier({ clients, onSave }) {
  ...
  const handleSave = () => { if (!mouvements.length) { setErreur("Ajoutez au moins un mouvement."); return; } onSave({ id: Date.now(), ...form, mouvements, dateCreation: today(), statut: mouvements.some(m => m.type === "sortie") ? "clos" : "ouvert" }); };
    ...
    <div>
      <div style={sf.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
          ...
          <div>
            <label style={sf.label}>Invoice Ref (référence client)</label>
            <input style={sf.input} placeholder="ex: ARS26OZ18248M" value={form.invoiceRef} onChange={e => setForm(f => ({ ...f, invoiceRef: e.target.value }))} />
          </div>
          <div>
            <label style={sf.label}>Notes internes</label>
            <input style={sf.input} placeholder="Passage douane, manution..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
          </div>
        </div>
      </div>
      <div style={sf.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div><label style={sf.label}>Dépotage vrac unité (nb colis)</label><input style={sf.input} type="number" min={0} value={form.depotage} onChange={e => setForm(f => ({ ...f, depotage: +e.target.value }))} /></div>
        </div>
      </div>
      <div style={sf.card}>
        ...
        <div style={{ background: "#fafaf8", border: "1px solid " + C.border, borderRadius: 8, padding: 16, marginBottom: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "130px 170px 90px 1fr auto", gap: 10, alignItems: "end" }}>
            ...
            <div><label style={sf.label}>Référence (BL, lettre de voiture…)</label><input style={sf.input} placeholder="Optionnel" value={mvt.ref} onChange={e => setMvt(v => ({ ...v, ref: e.target.value }))} /></div>
            <div><button style={sf.btn("accent")} onClick={addMvt}>+ Ajouter</button></div>
          </div>
        </div>
        {mouvements.length === 0 ? <div style={{ textAlign: "center", padding: "20px 0", color: C.muted, fontFamily: "sans-serif", fontSize: 13 }}>Aucun mouvement ajouté</div>
          : mouvements.map((m, i) => (
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button style={sf.btn("primary", { fontSize: 14, padding: "12px 28px" })} onClick={handleSave}>💾 Enregistrer le dossier</button>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={sf.card}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "sans-serif", fontSize: 13, marginBottom: 24 }}>
          <thead><tr style={{ background: C.bg }}>{["Désignation","Qté","PU HT","Total HT"].map((h,i) => <th key={h} style={{ padding: "9px 14px", textAlign: i===0?"left":"right", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: C.muted, borderBottom: "2px solid " + C.border }}>{h}</th>)}</tr></thead>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <div style={{ width: 300, fontFamily: "sans-serif" }}>
            ...
            {f.avecTVA ? <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid " + C.border, color: C.muted, fontSize: 14 }}><span>TVA 20%</span><span>{f.tva.toFixed(2)} €</span></div>
              : <div style={{ padding: "8px 10px", background: "#fef9c3", borderRadius: 4, fontSize: 12, color: "#92400e", fontWeight: 600, marginBottom: 4 }}>⚠️ Client exonéré de TVA — Facturation HT</div>}
...
function PageClients({ clients, setClients }) {
  ...
  const [erreur, setErreur] = useState("");
  const handleAjouter = () => { const nom = newClient.nom.trim().toUpperCase(); if (!nom) { setErreur("Nom obligatoire."); return; } if (clients.find(c => c.nom === nom)) { setErreur("Ce client existe déjà."); return; } setClients(cs => [...cs, { nom, tva: newClient.tva }]); setNewClient({ nom: "", tva: true }); setErreur(""); };
  const handleSaveEdit = (ancienNom) => { const nom = editing.nom.trim().toUpperCase(); if (!nom) return; if (nom !== ancienNom && clients.find(c => c.nom === nom)) { setErreur("Ce nom existe déjà."); return; } setClients(cs => cs.map(c => c.nom === ancienNom ? { nom, tva: editing.tva } : c)); setEditing(null); setErreur(""); };
  return (
    <div>
      <div style={sf.card}>
      ...
      <div style={sf.card}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "sans-serif", fontSize: 14 }}>
          <thead><tr style={{ background: C.bg }}>{["Nom","TVA","Facturation","Actions"].map(h => <th key={h} style={{ padding: "10px 16px", textAlign: "left", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.08em", color: C.muted, borderBottom: "2px solid " + C.border }}>{h}</th>)}</tr></thead>
...
function PageTarifs() {
  return <div style={sf.card}>
    ...
    <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "sans-serif", fontSize: 14 }}>
      ...
      <tbody>{[["Passage à quai","20 €","/ lot < 15 palettes — HT"],["Passage à quai","65 €","/ lot ≥ 15 palettes — HT"],["Cross dock entrée","4 €","/ palette — HT"],["Cross dock sortie","4 €","/ palette — HT"],["Stockage (3j offerts, J entrée+sortie exclus)","1,00 €","/ palette / jour ≤ 10j — HT"],["Stockage longue durée","0,70 €","/ palette / jour > 10j — HT"],["Palettisation","20 €","/ palette — HT"],["Dépotage vrac unité","1,17 €","/ colis — HT"],["Dépotage vrac complet","400 €","forfait — HT"],["Dépotage vrac rouleaux","Sur devis",""]].map(([p,t,u],i) => <tr key={i} style={{ borderBottom: "1px solid " + C.border, background: i%2===0?"#fff":C.bg }}><td style={{ padding: "10px 16px", fontWeight: 500 }}>{p}</td><td style={{ padding: "10px 16px", fontWeight: "bold", color: C.accent }}>{t}</td><td style={{ padding: "10px 16px", color: C.muted, fontSize: 12 }}>{u}</td></tr>)}</tbody>
    </table>
  </div>;
}
...
export default function DepotManager() {
  ...
  const [dossiers, setDossiersRaw] = useState(() => { try { return JSON.parse(localStorage.getItem("dm_dossiers") || "[]"); } catch { return []; } });
  const [clients, setClientsRaw] = useState(() => { try { return JSON.parse(localStorage.getItem("dm_clients") || "null") || CLIENTS_INIT; } catch { return CLIENTS_INIT; } });
  const setDossiers = (val) => { const next = typeof val === "function" ? val(dossiers) : val; setDossiersRaw(next); localStorage.setItem("dm_dossiers", JSON.stringify(next)); };
  const setClients = (val) => { const next = typeof val === "function" ? val(clients) : val; setClientsRaw(next); localStorage.setItem("dm_clients", JSON.stringify(next)); };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Georgia', serif", color: C.ink }}>
      <header style={{ background: C.ink, color: "#fff", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 62 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 11, color: "#00d97e", fontFamily: "sans-serif", display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 7, height: 7, borderRadius: "50%", background: "#00d97e" }} />Données sauvegardées</div>
